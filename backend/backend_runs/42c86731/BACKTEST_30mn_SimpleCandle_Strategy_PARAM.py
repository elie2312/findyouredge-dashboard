...
CSV_PATH = r"C:/Users/elieb/Desktop/Dashboard/backend_runs/42c86731/filtered_data.csv"  # Chargé depuis config.py
...
def resample_30m(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for sym, g in df.groupby("symbol"):
        ohlc = g[["open","high","low","close"]].resample("30min", label="right", closed="right").agg({
            "open":"first", "high":"max", "low":"min", "close":"last"
        })
        if "volume" in g.columns:
            vol = g[["volume"]].resample("30min", label="right", closed="right").sum()
...
class Trade:
    ...
    exit: Optional[float]  # Prix de sortie réel
    tp: Optional[float]
    sl: Optional[float]
    points: float
    pnl_usd: float
...
def simulate_day(day_df_30: pd.DataFrame, symbol_label: str) -> List[Trade]:
    ...
    for i in entry_idx:
        ...
        # Debug: afficher les données de la barre d'entrée (limité)
        if trades_done < 3:  # Seulement les 3 premiers trades pour éviter le spam
            print(f"[DEBUG] Trade #{trades_done+1} - Barre {bar['timestamp']}: O={bar['open']:.2f} C={bar['close']:.2f}")
        if bar["close"] > bar["open"]:
            direction = "long"
            entry = bar["close"] + slip
            sl = prev_bar["low"]
            if pd.isna(sl) or sl >= entry:
                print(f"  [SKIP] SL invalide: {sl:.2f} >= {entry:.2f}")
                continue  # skip si SL invalide
            r_pts = round_to_tick(entry - sl, TICK_SIZE)
            tp = entry + r_pts
            if trades_done < 3:
                print(f"  [LONG] Entry={entry:.2f}, SL={sl:.2f}, TP={tp:.2f}, R={r_pts:.2f}")
            ...
            tp = entry - r_pts
            if trades_done < 3:
                print(f"  [SHORT] Entry={entry:.2f}, SL={sl:.2f}, TP={tp:.2f}, R={r_pts:.2f}")
        else:
            # doji: pas de trade
            continue
        ...
        if exit_time is None:
            ...
            result = "EOD"
            if trades_done < 3:
                print(f"  [EOD] Exit a {exit_time} au prix {exit_price:.2f}")
        else:
            if trades_done < 3:
                print(f"  [{result}] Exit a {exit_time} au prix {exit_price:.2f}")
        ...
        trades.append(Trade(
            ...
            direction=direction,
            entry=float(entry),
            exit=float(exit_price),  # Prix de sortie réel
            tp=float(tp),
            sl=float(sl),
            points=float(points),
            pnl_usd=float(pnl),
